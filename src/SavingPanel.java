 ...
 * @date Fall 2019
 */
public class SavingPanel extends JFrame implements ActionListener {
    ...
    public SavingPanel(int frameWidth, int frameHeight,
                       Map<Long, Customer> customers,Map<Long, List<Transaction>> transactions,
                       Map<Long, List<Account>> accounts, Customer[] customer) {
        super("Open a new Saving Account");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (cmd.equals(OPEN_LABEL)) {
            ...
            Account saving = new SavingAccount(customerName, depositAmount);
            List<Account> account = this.accounts.getOrDefault(phoneNumber, new ArrayList<>());
            account.add(saving);
            this.accounts.put(phoneNumber, account);
            Transaction transaction = new Transaction(customer, saving);
